    ...
    def parse_articles(self, response):
        ...
        if len(articles) != 0:
            for article in articles:
                ...
                str_read_time = article.xpath('.//*[@class="readingTime"]/@title').get()
                if str_read_time != None:
                    int_read_time = str_read_time.split()[0]

                collection = article.xpath('.//a[@data-action="show-collection-card"]/text()').get()
                
                title = article.xpath('.//*[contains(@class, "title")]/text()').get()
                ...
                subtitle_preview = article.xpath('.//*[@name="previewSubtitle"]/text()').get()
                ...
                article_url = article.xpath('.//a[contains(@class, "button--smaller")]/@href').get()
                if article_url != None:
